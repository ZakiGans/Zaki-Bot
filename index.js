...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'FN:ZAKIGANS\n' // NAMA OWNER BOT
            + 'ORG:Creator ZAKI ID;\n' // KASIH CREDITS GUA ZakiGans Tod!!!
            + 'TEL;type=CELL;type=VOICE;waid=6283144780782:+62 831-4478-0782\n' // GANTI NOMOR LU
            + 'END:VCARD'
prefix = '#'
blocked = []

const Apikey = 'BELI  BIAR WORK' // APIKEY DOWNLOAD FILE
const VhtearKey = 'RamlanID' // YANG DI YOUTUBE
const TobzKey = 'BotWeA' // JANGAN MAU ENAK DOANG
const BarBarKey = 'BELI BIAR WORK'
...
var query = body.slice(5)
                data = await fetchJson(`https://api.vhtear.com/ytmp3?query=${query}&apikey=${VhtearKey}`, {method: 'get'})
			...
			case 'owner':
				...
				case 'galaxtext':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/galaxytext?text=${teks}&apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'primbonjodoh':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					anu = await fetchJson(`https://api.vhtear.com/primbonjodoh?nama=${gbl1}&pasangan=${gbl2}&apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'ramaljadian':
					...
					var gbl2 = gh.split("|")[1];
					var gbl3 = gh.split("|")[2];
					anu = await fetchJson(`https://api.vhtear.com/harijadian?tgl=${gbl1}&bln=${gbl2}&thn=${gbl3}&apikey=${VhtearKey}`, {method: 'get'})
					reply(anu.result.hasil)
					break
                case 'tahta':
            var gh = body.slice(9)
                ...
                reply(mess.wait)
					...
					client.sendMessage(from, bufferrty, image, {quoted: mek, caption: 'subrek Ikyy_ads_' })
					break
					case 'iri':
...
break
				...
				case 'lovemake':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${love}&apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'thunder':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/thundertext?text=${thun}&apikey=${VhtearKey}`, {method: 'get'})
                ...
                case 'stiltext':
                      ...
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
                      buff = await getBuffer(`https://api.vhtear.com/silktext?text=${gl1}&text2=${gl2}&apikey=${VhtearKey}`, {method: 'get'})
                      reply(mess.wait)
				...
				case 'testing2':
					...
					if (args.length < 1) return reply('Teks nya mana?')
					reply(mess.wait)
					party = await getBuffer(`https://api.vhtear.com/partytext?text=${coli1}&text2=${coli2}&apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'party':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/partytext?text=${part}&apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'rtext':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/romancetext?text=${tels5}&apikey=${VhtearKey}`, {method: 'get'})
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				case 'walpaperhd':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/walpaper?query=${teks}&apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'wibu':
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${VhtearKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'randomcat':
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomcat?apikey=${VhtearKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'mlherolist':
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/mlherolist?apikey=${VhtearKey}`, {method: 'get'})
				...
				case 'ramalhp':
					if (args.length < 1) return reply('teks nya mana om')
					kj = body.slice(12)
					anu = await fetchJson(`https://api.vhtear.com/nomerhoki?no=${kj}&apikey=${VhtearKey}`, {method: 'get'})
					reply(anu.result.hasil)
					break
				case 'textscreen':
					if (args.length < 1) return reply('teks nya mana om')
					tels = body.slice(9)
					anu = await fetchJson(`https://api.vhtear.com/textscreen?query=${tels}&apikey=${VhtearKey}`,{method: 'get'})
			...
			var gh = body.slice(9)
               ...
               case 'infogempa':
                ...
                case 'infogithub':
                   ...
                   break
                case 'infocuaca':
                   anu = await fetchJson(`http://tobz-cuaca.herokuapp.com/?menu=cuaca&wilayah=${body.slice(6)}&apikey=${BarBarKey}`, {method: 'get'})
                   if (anu.error) return reply(anu.error)
                ...
                case 'pokemon':
					...
					break
				case 'inu':
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=inu&apikey=${VhtearKey}`,{method: 'get'})
				...
				case 'kurumi':
					if (!isAnime) return reply('❌ *Harus Mengaktifkan Mode Anime* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=kurumi&apikey=${VhtearKey}`,{method: 'get'})
				...
				case 'hentai':
					...
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=animehentai&apikey=${VhtearKey}`,{method: 'get'})
				...
				case 'ytmp':
					if (args.length < 1) return reply('Urlnya mana um?')
					anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${body.slice(7)}&apikey=${VhtearKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Title* : ${anu.title}\n*Filesize* : ${anu.size}`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
				...
				case 'tulis':
					...
					tulis = body.slice(6)
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/write?text=${tulis}&apikey=${VhtearKey}`, {method: 'get'})
			    ...
			    case 'lovemake':
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buff = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey=${VhtearKey}`,{method: 'get'})
                    client.sendMessage(from, buff, image, {quoted: mek, caption: `${teks}`})
			     	break
			    case 'shorturl':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *IKYY ID BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
